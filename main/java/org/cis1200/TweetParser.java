...
public class TweetParser {
    ...
    static String extractColumn(String csvLine, int csvColumn) {
        ...
        if (list.length <= csvColumn || csvColumn < 0) {
            return null;
        }
    ...
    static List<String> csvDataToTweets(BufferedReader br, int tweetColumn) {
        ...
        while (li.hasNext()) {
            list.add(extractColumn(li.next(), tweetColumn));
        }
        list.removeAll(Collections.singleton(null));
        list.removeAll(Collections.singleton(""));
        if (tweetColumn >= list.size()) {
            return emptyList;
        }
        return list;
    }
    ...
    static List<String> parseAndCleanSentence(String sentence) {
        ...
        for (String s : arrayList) {
            arrayList.set(arrayList.indexOf(s), cleanWord(s));
        }
    ...
    static List<List<String>> parseAndCleanTweet(String tweet) {
        ...
        sentences = tweetSplit(tweet);
        for (String s : sentences) {
            List<String> toAdd = parseAndCleanSentence(s);
            if (!toAdd.isEmpty()) {
                list.add(toAdd);
            }
        }
    ...
    public static List<List<String>> csvDataToTrainingData(
        ...
        ArrayList<List<String>> list = new ArrayList<>();
        while (li.hasNext()) {
            String nextS = extractColumn(li.next(), tweetColumn);
            if (nextS != null) {
                tweets.add(nextS);
            }
        }
        for (String s : tweets) {
            ...
            for (List<String> str : parseAndCleanTweet(s)) {
