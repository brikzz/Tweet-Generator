...
public class MarkovChainTest {
    ...
    @Test
    public void testTrainMultipleEntries() {
        MarkovChain mc = new MarkovChain();
        String sentence = "1 2 1 2";
        mc.train(Arrays.stream(sentence.split(" ")).iterator());
        ...
        ProbabilityDistribution<String> pd2 = mc.chain.get("2");
        assertTrue(pd2.getRecords().containsKey("1"));
        assertEquals(1, pd2.count("1"));
        ProbabilityDistribution<String> pd3 = mc.chain.get("2");
        assertTrue(pd3.getRecords().containsKey(MarkovChain.END_TOKEN));
        assertEquals(1, pd3.count(MarkovChain.END_TOKEN));
    }

    @Test
    public void testTrainAllTheSame() {
        MarkovChain mc = new MarkovChain();
        String sentence = "1 1 1 1 1";
        mc.train(Arrays.stream(sentence.split(" ")).iterator());
        assertEquals(1, mc.chain.size());
        ProbabilityDistribution<String> pd1 = mc.chain.get("1");
        assertTrue(pd1.getRecords().containsKey("1"));
        assertEquals(4, pd1.count("1"));
        ProbabilityDistribution<String> pd3 = mc.chain.get("1");
        assertTrue(pd3.getRecords().containsKey(MarkovChain.END_TOKEN));
        assertEquals(1, pd3.count(MarkovChain.END_TOKEN));
    }
    ...
    public void testTrainEmptyIterator() {
        ...
        assertFalse(pd1.getRecords().containsKey(""));
    }
    ...
    @Test
    public void testWalkDifferentStartingCapitalization() {
        String[] expectedWords = { "Hello", "world" };
        MarkovChain mc = new MarkovChain();

        String sentence1 = "Hello world bye";
        String sentence2 = "World says hello";
        mc.train(Arrays.stream(sentence1.split(" ")).iterator());
        mc.train(Arrays.stream(sentence2.split(" ")).iterator());
